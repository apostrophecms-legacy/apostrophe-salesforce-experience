...
module.exports = {
  ...
  construct: async function (self, options) {
    ...
    self.profileCallback = async function (profile, callback) {
      const assertionXml = await profile.getAssertionXml ? profile.getAssertionXml() : false;

      if (!assertionXml) {
        console.error('No assertion XML returned.');
      } else {
        const assertionBuffer = Buffer.from(assertionXml);
        const assertion64 = assertionBuffer.toString('base64');
