...
module.exports = function(self, options) {
  ...
  self.connect = async function() {
    ...
    await conn.login(self.options.options.adminUsername || 'USERNAME', self.options.adminPasswordAndToken || 'PASSWORD');
  ...
  // as specified by the `experienceQueries` option of the module, an
  // array of objects with `userSoql`, `soql`, `labelField` and `idField`
  // properties.
  //
  ...
  // Appropriate escaping is done.
 
  self.getExperiences = async function(username) {
    const conn = await self.connect(options);

    const experiences = [];
    for (const query of self.options.experienceQueries) {
      let result;
      if (username) {
        result = await conn.query(query.userSoql.replace(':username', escapeSOQOL(username)));
      } else {
